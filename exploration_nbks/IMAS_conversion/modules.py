...
def f_linear(old_gk_dict,new_gk_dict): 
    ...
    par_strg = xmltodict.unparse({'root':old_gk_dict['code']['parameters']},pretty=True)
    if not wavevector[0]['eigenmode']: 
        dict_c['wavevector'][0]['eigenmode'].append({'code':{'paramters':par_strg}})
    else: 
        dict_c['wavevector'][0]['eigenmode'][0]['code']['parameters'] = par_strg
...
if __name__=="__main__":
    ...
    top_dir= 'src/mgkdb/exploration_nbks/imas_compare_david/'
    fname=top_dir+'gyrokinetics_old_1_tracer_efit.yaml'
    with open(fname,'r') as f:
        gk_dict = yaml.safe_load(f)
        
    fname=top_dir+'gyrokinetics_new_1_tracer_efit.yaml'
    with open(fname,'r') as f:
        GK_dict = yaml.safe_load(f)
