...
def update_mongo(out_dir, db, runs_coll):
    ...
    if update_option == '0':
        ...
        affect_QoI = input('Will the file change QoIs or Diagnostics? (Y/N)')
        updated = []
...
def upload_file_chunks(db, out_dir, par_file, large_files=False, extra_files=False):
    '''
...
def upload_linear(db, out_dir, par_file, user, linear, confidence, input_heat, keywords, 
...
def upload_nonlin(db, out_dir, par_file, user, linear, confidence, input_heat, keywords, 
                  large_files=False, extra=False, verbose=True):
